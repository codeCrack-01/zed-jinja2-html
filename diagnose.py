...
def check_extension_files() -> Dict[str, bool]:
    ...
    required_files = {
        ...
        'language config': base_path / 'languages' / 'jinja2-html.toml',
        'grammar file': base_path / 'grammars' / 'jinja2-html' / 'grammar.json',
...
def validate_extension_manifest() -> Tuple[bool, List[str]]:
    ...
    try:
        ...
        if 'language_servers.jinja2-html-lsp' not in content:
            issues.append("Missing language server configuration")
        
        if 'languages.jinja2-html' not in content:
            issues.append("Missing language configuration")
...
def validate_grammar_file() -> Tuple[bool, List[str]]:
    ...
    base_path = Path(__file__).parent
    grammar_path = base_path / 'grammars' / 'jinja2-html' / 'grammar.json'
